...
pub fn modsub(a: u64, b: u64, q: u64) -> u64 {
    return if a >= b { a.wrapping_sub(b) } else { (q + a).wrapping_sub(b) };
}
...
pub fn barrett_precompute_old(q: u64) -> (u64, u64) {
    let logq: u64 = if q==0 { 0 } else { 64 - (q.leading_zeros() as u64) };
...
impl CongruenceClass {
    ...
    pub fn modmul_eq(&self, a: &mut u64, b: u64) {
        ...
        *a = if r < self.q { r } else { r.wrapping_sub(self.q) };
    }

    pub fn modadd(&self, a: u64, b: u64) -> u64 {
        let t = a + b;
        return if t <= self.q { t } else { t.wrapping_sub(self.q) };
    }

    pub fn modadd_eq(&self, a: &mut u64, b: u64) {
        let t = *a + b;
        *a = if t <= self.q { t } else { t.wrapping_sub(self.q) };
    }

    pub fn modsub(&self, a: u64, b: u64) -> u64 {
        return if a >= b { a.wrapping_sub(b) } else { (self.q + a).wrapping_sub(b) };
    }

    pub fn modsub_eq(&self, a: &mut u64, b: u64) {
        *a = if *a >= b { (*a).wrapping_sub(b) } else { (self.q + *a).wrapping_sub(b) };
    }
