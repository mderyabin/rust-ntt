...
fn benchmark_modmul_barrett_old(c: &mut Criterion) {
    ...
    c.bench_function("modmul barrett old", |b| {
        b.iter(|| modmul_barrett_old(black_box(in1), black_box(in2), black_box(Q), black_box(mu), black_box(logq))) 
    });
}
...
fn benchmark_modmul_barrett_old_eq(c: &mut Criterion) {
    ...
    c.bench_function("modmul barrett old eq", |b| {
        b.iter(|| modmul_barrett_old_eq(&mut in1, black_box(in2), black_box(Q), black_box(mu), black_box(logq))) 
    });
}
